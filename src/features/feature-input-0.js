// Input handling for ReminderBot.
...
class InputHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'input' + ' payload');
 return { 'input': String(payload).slice(0, 200) };
 }
...
module.exports = { InputHandler, run: (payload, opts) => new InputHandler().run(payload, opts) };